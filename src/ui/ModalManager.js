class ModalManager {
    ...
    showLevelUpModal() {
        ...
        const pool = [
            ...
            { name: '바이탈 코어', desc: '최대 체력 +20 및 체력 20 회복', fn: () => { playerStats.maxHp += 20; playerStats.maxHpLevel++; playerStats.hp = Math.min(playerStats.maxHp, playerStats.hp + 20); scene.uiManager.updateHPUI(); } },
            { name: '긴급 복구', desc: '체력 60% 즉시 복구', fn: () => { playerStats.hp = Math.min(playerStats.maxHp, playerStats.hp + 60); scene.uiManager.updateHPUI(); } }
        ];
