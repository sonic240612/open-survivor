...
class MainScene extends Phaser.Scene {
    ...
    simulateEntities(dx, dy, dt, mins) {
        ...
        this.enemies.getChildren().forEach(e => {
            ...
            let curSpd = Math.min(CONSTANTS.ABSOLUTE_MAX_ENEMY_SPEED, CONSTANTS.BASE_ENEMY_SPEED + (mins * 8) + (playerStats.currentLevel * 2.5));
    ...
    tryShooterFire(enemy) {
        const now = this.time.now;
        if (now - enemy.lastShot < 1800 || now - gameState.lastShooterFire < 260) return;
        ...
        this.physics.velocityFromRotation(ang, 230, shot.body.velocity);
        enemy.lastShot = now;
    ...
    spawnEnemy() {
        ...
        const isElite = !config.noElite && Math.random() * 100 < Math.min(18, Math.floor(mins) * 3);
        ...
        const waveScale = 1 + (Math.min(mins, 15) * 0.15) + (playerStats.currentLevel * 0.035);
    ...
    getSpawnRate(mins) {
        let rate;
        if (mins < 3) rate = 1300 - mins * 90;
        else if (mins < 7) rate = 1050 - (mins - 3) * 95;
        else if (mins < 12) rate = 720 - (mins - 7) * 55;
        else rate = 470 - Math.min(mins - 12, 8) * 28;

        const seconds = Math.floor(gameState.gameTime / 1000);
        const bossSoon = seconds > 0 && (600 - (seconds % 600)) <= 30;
        if (bossSoon) rate += 180;

        return Math.max(180, Math.floor(rate - playerStats.currentLevel * 6));
    }
    ...
    pickEnemyType(mins) {
        const pool = mins < 3
            ...
            : mins < 7
                ? ['normal', 'runner', 'runner', 'tanker', 'splitter']
                : mins < 12
                    ? ['normal', 'runner', 'tanker', 'splitter', 'splitter', 'shooter']
                    : ['runner', 'tanker', 'splitter', 'shooter', 'shooter', 'buffer'];
        return Phaser.Utils.Array.GetRandom(pool);
    }

    getEnemyConfig(type) {
        const configs = {
            normal: { tex: 'e_tex', size: 32, hp: CONSTANTS.BASE_ENEMY_HP, speedMult: 1, expBonus: 0 },
            runner: { tex: 'runner_tex', size: 30, hp: 8, speedMult: 1.45, expBonus: 1 },
            tanker: { tex: 'tanker_tex', size: 48, hp: 42, speedMult: 0.58, expBonus: 5 },
            splitter: { tex: 'splitter_tex', size: 36, hp: 20, speedMult: 0.9, expBonus: 3 },
            shooter: { tex: 'shooter_tex', size: 36, hp: 18, speedMult: 0.72, expBonus: 4 },
            buffer: { tex: 'buffer_tex', size: 40, hp: 30, speedMult: 0.68, expBonus: 6, noElite: true }
        };
