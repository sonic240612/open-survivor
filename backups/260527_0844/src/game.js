...
class MainScene extends Phaser.Scene {
    ...
    hitEnemy(enemy, damage, type) {
        ...
        else if (type === 'lightning') color = '#00ffff';
        ...
        // 3. 피격 스파크 파티클 튐
        this.sparkEmitter.emitParticleAt(enemy.x, enemy.y, type === 'lightning' ? 12 : 5);
    ...
    updateWeaponDashboard() {
        ...
        const weaponTypes = ['자동 소총'];
        if (playerStats.lightningCount > 0) weaponTypes.push('낙뢰');
        if (playerStats.swordCount > 0) weaponTypes.push('위성 검');
